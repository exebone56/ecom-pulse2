...
const RevenueMarketplace = () => {
    ...
    const loadMarketplaces = async () => {
      try {
        setMarketplaces([
          ...
          { id: '1', name: 'Wildberries', color: '#FF6B6B', code: 'wildberries' },
          { id: '2', name: 'OZON', color: '#005BFF', code: 'ozon' },
          { id: '3', name: 'Яндекс.Маркет', color: '#FFD700', code: 'yandex_market' },
        ]);
      } catch (err) {
  ...
  const loadChartData = async (marketplaceId) => {
    try {
      setLoading(true);
      setError(null);
      // Передаем одиночный ID вместо массива
      const data = await analyticsApi.getRevenueChartData([marketplaceId]);
      setChartData(data);
    } catch (err) {
    ...
    <AnalyticBlock
      ...
      subtitle={
        <div className="flex items-center justify-between w-full">
          <span>Отдельно по каждому маркетплейсу</span>
    ...
    >
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {/* Компактный белый селектор - теперь одиночный выбор */}
...
export default RevenueMarketplace;